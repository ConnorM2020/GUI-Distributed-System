...
class Node:
    ...
    def process_transaction(self, txn):
        ...
        if txn.id in self.processed_transaction_ids:
            self.log("Duplicate Broadcast", f"Transaction {txn.id} already processed.")
            return False
        ...
        # Increment the Lamport clock after processing the transaction
    ...
    def broadcast_transaction(self, txn):
        ...
        # Auto-sync transactions after broadcasting
        self.auto_sync_with_delay()
        return success
    ...
    def merge_transactions(self, peer_transactions):
        ...
        for txn_data in peer_transactions:
            ...
            # Update the transaction counter to avoid duplicate counters
            with self.counter_lock:
                self.transaction_counter = max(self.transaction_counter, txn_index + 1)

        self.log("Transactions", "Transaction list merged successfully.")
    ...
    def get_node_details(self):
        ...
        if self.transactions:
            for txn in self.transactions.values():
                ...
                if txn.custom_fields:  # Check if there are custom fields
                    custom_fields_text = ", ".join(f"{key}: {value}" for key, value in txn.custom_fields.items())
                    txn_text += f" (Custom Fields: {custom_fields_text})"
                tk.Label(transaction_frame, text=txn_text, font=("Arial", 10)).pack(anchor="w", padx=10, pady=2)
        else:
