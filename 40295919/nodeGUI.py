...
class NodeGUI:
    ...
    def remove_peer(self):
        """Remove a specific peer from the node's peer list."""
        peer = simpledialog.askstring("Remove Peer", "Enter peer address (IP:PORT):")
        if not peer:
            self.log_message("No peer address provided.")
            return

        # Check if the entered peer is the current node itself
        if peer == self.node.address:
            self.log_message("Error: Cannot remove the current node itself.")
            return

        # Check if the peer is in the peer list
        if peer in self.node.peers:
            self.node.remove_peer(peer)
            self.log_message(f"Successfully removed peer: {peer}")
        else:
            self.log_message(f"Error: Peer '{peer}' not found in the list.")
    ...
    def export_data(self):
        ...
        with open(file_path, "w", newline="") as file:
            ...
            # Add headers
            writer.writerow(["ID", "Sender", "Receiver", "Amount", "Timestamp", "Custom Fields"])
            
            # Write each transaction, including custom fields
            for txn in self.node.transactions.values():
                # Convert custom fields to JSON string for better readability
                custom_fields_str = json.dumps(txn.custom_fields) if txn.custom_fields else "None"
                writer.writerow([txn.id, txn.sender, txn.receiver, txn.amount, txn.timestamp, custom_fields_str])
